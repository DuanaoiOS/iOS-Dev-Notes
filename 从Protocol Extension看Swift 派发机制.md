...
## Protocol中的诡异现象
 	相信大家都知道，在Swift Protocol中是可以通过Extension提供默认实现方法的，并且无需在声明中定义。这也是Swift Protocol如此强大的基石，广大的Swifter们可以利用灵活的特点轻松实现POP，不过此文重点不是介绍如何面向协议编程，有兴趣的同学可以参阅喵神出品的[面向协议编程与 Cocoa 的邂逅 (上)](https://onevcat.com/2016/11/pop-cocoa-1/)
	下文要介绍是在使用Protocol开发中我们可能会遇到（也许遇到过）的坑，比如使用Protocol作为类型声明、泛型约束，并且调用其Extension中的默认实现，就有可能出现不符合我们预期的结果。如下示例：(其中注释为对应语句输出结果)
...
> 1. Swift采用直接派发的方式对方法调用的性能带来了提升，同时也损失了部分Runtime的动态特性  
>   
> 2. Swift在NSObject派生类中使用函数表派发，此举是否真正的带来了性能上的提升，并且在Extension中仍是采用消息机制，对这个做法存在疑惑  
>   
> 3. 通过Selector绑定的方法或KVO监听的属性值还需要显示修饰成`@objc`或`dynamic`， 在编译器优化上是否可以检测到自动注册到Runtime中去？  
>   
